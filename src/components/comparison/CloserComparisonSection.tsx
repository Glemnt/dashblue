...
export const CloserComparisonSection = ({ data }: Props) => {
  return (
    <Card className="bg-card border-border">
      <CardHeader>
        <CardTitle className="text-foreground">Comparação de Performance Closer</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2">
              {data.top3Current.map((closer, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-primary/10 rounded border border-primary/20">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-foreground">{closer.nome}</span>
                  </div>
                  <span className="font-semibold text-foreground">{formatarReal(closer.receitaTotal)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {data.top3Previous.map((closer, idx) => (
